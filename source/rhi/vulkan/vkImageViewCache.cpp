...
bool viewKey::operator==(const viewKey& o) const noexcept
{
	return image == o.image && format == o.format && aspect == o.aspect &&
		mip == o.mip && base_layer == o.base_layer && layer_count == o.layer_count;
}

size_t viewKeyHash::operator()(const viewKey& k) const noexcept
{
    size_t h = std::hash<uint64_t>()((uint64_t)k.image);
    h ^= std::hash<uint64_t>()(((uint64_t)k.format << 32) | k.aspect) + 0x9e3779b9 + (h << 6) + (h >> 2);
    h ^= std::hash<uint32_t>()(k.mip) + 0x9e3779b9 + (h << 6) + (h >> 2);
    h ^= std::hash<uint32_t>()(k.base_layer) + 0x9e3779b9 + (h << 6) + (h >> 2);
    h ^= std::hash<uint32_t>()(k.layer_count) + 0x9e3779b9 + (h << 6) + (h >> 2);
    return h;
}
...
VkImageView vkImageViewCache::get(const renderTargetViewDesc* desc)
{
    auto vk_tex = static_cast<vkTexture*>(desc->tex);
    const VkImage image = vk_tex->get_image();
    const VkFormat vk_format = vk_tex->get_format();
    VkImageAspectFlags aspect = vk_aspect_from_format(desc->tex->desc.format);
...
VkImageView vkImageViewCache::get_or_create(const viewKey& key, const VkComponentMapping* swizzle) 
{
    ...
    VkImageViewCreateInfo create_info{
        .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
        .image = key.image,
        .viewType = (key.layer_count > 1) ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D,
        .format = key.format,
        .subresourceRange = sub_res_range
    };
    if (swizzle) 
